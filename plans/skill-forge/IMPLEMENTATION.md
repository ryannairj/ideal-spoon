...
2. **Brainstorm mode** — the evidence scan procedure (ordered list of sources per §0 with what to extract from each: e.g. package scripts → "tasks run often but undocumented"); ranking formula in prose: `score = frequency-evidence × automation-absence`; output format: a markdown table `| # | Skill name | Archetype | One-liner | Evidence (path) | Effort |` capped at 7 rows, each row followed by nothing — end with "Reply with a number to build it."
   Hard rules: every proposal MUST cite a real path found in the scan; before proposing, list existing skills and drop overlaps.
...
**M1** — T1 brainstorm mode + evidence scan rules; T2 remaining 3 archetypes + interview.md; T3 description-guide.md + rubric.md finalized; T4 fixture repos (`fixtures/rich-repo/` with runbooks+scripts, `fixtures/bare-repo/`) + expected-proposal checklists (review doc, since skills aren't unit-testable: a `TESTING.md` protocol with expected outcomes per fixture).
